...
unordered_map<string,node*> mymap_e; 
...
/***************************************backword neighur finding************************************/
//find in hash map
r_pair check_hashmap_e(string key)
{
	r_pair ck;
	unordered_map<string,node*>::const_iterator got = mymap_e.find (key);
	if(got == mymap_e.end())
	{
		ck.tval = false;
	}
...
list<node*> get_neighbour_puzzle_e(node* b){
	list<node*> mylist;
	cout<<"hash key :  "<<hash_key_n(b)<<endl<<endl;
	//node* n1 = new node();
	//n1->id = b->id;
	...
	for(int i=0;i<3;i++){
		...
		if(index_validity(pos1-1, pos2)){
			...
			v_n[pos1-1][pos2]=-1;
			string v_ns= hash_key_v(v_n);
			r_pair v_np =check_hashmap_e(v_ns);
			if(v_np.tval)
			{
				mylist.push_back(v_np.n);			
			}
		...
		if(index_validity(pos1+1, pos2)){
			...
			v_s[pos1+1][pos2]=-1;
			string v_ss= hash_key_v(v_s);
			r_pair v_sp =check_hashmap_e(v_ss);
			if(v_sp.tval)
			{
				mylist.push_back(v_sp.n);			
			}
		...
		if(index_validity(pos1, pos2-1)){
			...
			v_w[pos1][pos2-1]=-1;
			string v_ws= hash_key_v(v_w);
			r_pair v_wp =check_hashmap_e(v_ws);
			if(v_wp.tval)
			{
				mylist.push_back(v_wp.n);			
			}
		...
		if(index_validity(pos1, pos2+1)){
			...
			v_e[pos1][pos2+1]=-1;
			string v_es= hash_key_v(v_e);
			r_pair v_ep =check_hashmap_e(v_es);
			if(v_ep.tval)
			{
				mylist.push_back(v_ep.n);			
			}
